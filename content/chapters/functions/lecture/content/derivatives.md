...
$$\begin{aligned} \frac{f(x+h)-f(x)}{h}&=\frac{(x+h)^n-x^n}{h}\\ &=\frac{\sum_{q=0}^{n-1}\binom{n}{q}x^qh^{n-q}}{h}\\ &=\sum_{q=0}^{n-1}\binom{n}{q}x^qh^{n-q-1}\to\binom{n}{n-1}x^{n-1}=nx^{n-1}\end{aligned}$$
...
The derivatives of the exponential function is the exponential function itself i.e
...
If $f(x) = 2+3x$ and $g(x)+x^3$ \ then we know that\ $f'(x)=3$, $g(x)=3x^2$ and if we write
...
$$\begin{aligned} f(x)&=&\frac{dx}{dx}e^x+x\frac{de^x}{dx}+\frac{dx^2}{dx}\ln x +x^2\frac{d \ln x}{dx}\\ &=&1\cdot e^x + x \cdot e^x + 2x \cdot \ln x + x^2 \cdot \frac{1}{x}\\ &=&e^x \left ( 1+x \right ) + 2x \ln x +x\end{aligned}$$

:::

## Derivatives of Composite Functions

If $f$ and $g$ are functions and $h=f \circ g$ so that\ $h(x) = f(g(x))$ then\ $h'(x) = \frac{dh(x)}{dx} = f'(g(x)) g'(x)$
...
For fixed $x$ and $y$ consider $f(b) = (y-bx)^2$ \ Then the derivative is computed as follows:
