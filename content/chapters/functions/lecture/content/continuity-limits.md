...
The probability of landing exactly $x$ heads, $P(X=x)$, is $p(x) = \binom{n}{x} p^n (1-p)^{n-x}$.
The probabilities are

$$\begin{array}{c c c} \hline x & p(x) & F(x)\\ \hline 0 & \frac{1}{8} & \frac{1}{8}\\ 1 & \frac{3}{8} & \frac{4}{8}\\ 2 & \frac{3}{8} & \frac{7}{8}\\ 3 & \frac{1}{8} & 1\\ \hline \end{array}$$
...
If $f(x)$ gets steadily closer to some number $c$ as $x$ gets closer to a number $x_0$, then $c$
...
As $x$ increases, $g(x)$ decreases, approaching $0$ but never getting there since $\frac{1}{x}=0$ has no solution.
...
$$\lim_{S\to\infty}\frac{\alpha S}{1+\frac{S}{K}} = \lim_{S\to\infty}\frac{\alpha }{\frac{1}{S}+\frac{1}{K}} =\alpha K.$$
...
This is seen from considering the components:\ Since $e^{-x} = \frac{1}{e^{x}}$ and the exponential function goes to infinity as $x\to\infty$, $e^{-x}$ goes to $0$ and hence $f(x)$ goes to 1
...
Since there is an $x$ in the denominator we cannot just direct substitute the 0 as $x$ sunce that would give us $\frac{0}{0}$, which is an indeterminate form.
